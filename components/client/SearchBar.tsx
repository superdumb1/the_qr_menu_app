...
const SearchBar = () => {
    ...
    return (
        <div className="w-full group">
            ...
            {!searchQuery && (
                <div className="absolute top-[calc(100%+12px)] left-0 w-full flex gap-4 px-2 overflow-x-auto no-scrollbar z-10">
                    {['Momo', 'Biryani', 'Pizza'].map((tag) => (
                        <button
                            key={tag}
                            onClick={() => setSearchQuery(tag)}
                            className="text-[9px] font-black uppercase tracking-widest text-text-muted/50 hover:text-primary transition-colors flex-shrink-0"
                        >
                            #{tag}
                        </button>
                    ))}
